        ...
        {data.photoUrl ? (
          ...
          <>
            ...
            <span style={{ color: '#a1a1aa', fontSize: 'var(--text-sm)' }}>
              Tap to upload a photo (Crucial for identification)
            </span>
          </>
        )}
      </div>

      <FormField label={t('approximateName')} helperText={t('approximateNameHelper')}>
        <Input 
          value={data.fullName || ''}
          onChange={(e) => updateData({ fullName: e.target.value })}
          placeholder={t('approximateNamePlaceholder')}
        />
      </FormField>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
        <FormField label={t('approximateAge')} required>
          <Input 
            type="number"
            value={data.age || ''}
