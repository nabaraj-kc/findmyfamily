...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'hard_purge_all') {
      const summary = hardPurgeAllData();
    ...
    if (action === 'delete_selected_gallery' && Array.isArray(imageIds)) {
      const count = deleteGalleryImagesBulk(imageIds);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (action === 'update_tip' && tipId && tipStatus) {
      const updated = updateTipStatus(tipId, tipStatus);
      return NextResponse.json({ success: updated });
    }

    if (caseId && status) {
      const updated = updateCaseStatus(caseId, status);
      return NextResponse.json({ success: updated });
    }
